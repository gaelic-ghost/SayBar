...
`SpeakSwiftlyServer` 8.0.4 provides the app-facing `@Observable` object that a host app is supposed to own. That surface includes:
...
SayBar is intentionally not adopting the standalone-install helper surface yet. In `SpeakSwiftlyServer` 8.0.4, that helper surface remains outside the embedded library contract SayBar imports. The current product baseline is still the embedded runtime that lives inside the app process.
